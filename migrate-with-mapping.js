...
async function migrateWithMapping() {
  try {
    console.log('🔄 Starting data migration with ID mapping...');
    
    // ID mapping from old to new
    const userIdMap = new Map();
    
    // Migrate users first and create ID mapping
    if (existingData.users && existingData.users.length > 0) {
      console.log(`📋 Migrating ${existingData.users.length} users...`);
      
      for (const [oldId, user] of existingData.users) {
        try {
          ...
          console.log(`  ✅ Migrated user: ${user.name} (Old ID: ${oldId} → New ID: ${newId})`);
        } catch (error) {
    ...
    // Migrate user activities with mapped IDs
    if (existingData.userActivities && existingData.userActivities.length > 0) {
      ...
      for (const [id, activity] of existingData.userActivities) {
        try {
          ...
          if (!newUserId) {
            console.log(`  ⚠️ Skipping activity ${activity.activityName} - user ${activity.userId} not found`);
            continue;
          }
          ...
          `, [
            newUserId, activity.activityName, activity.skillLevel, 
            activity.isPrimary, activity.coachingExperienceYears, activity.certifications
          ]);
          
          console.log(`  ✅ Migrated activity: ${activity.activityName} for user ${newUserId}`);
        } catch (error) {
    ...
    // Migrate interests with mapped IDs
    if (existingData.interests && existingData.interests.length > 0) {
      ...
      for (const [id, interest] of existingData.interests) {
        try {
          const newSenderId = userIdMap.get(interest.senderId);
          const newReceiverId = userIdMap.get(interest.receiverId);
          
          if (!newSenderId || !newReceiverId) {
            console.log(`  ⚠️ Skipping interest - sender ${interest.senderId} or receiver ${interest.receiverId} not found`);
            continue;
          }
          ...
          `, [
            newSenderId, newReceiverId, interest.status, 
            interest.createdAt || new Date().toISOString(), interest.updatedAt
          ]);
          
          console.log(`  ✅ Migrated interest: ${newSenderId} → ${newReceiverId} (${interest.status})`);
        } catch (error) {
    ...
    console.log('🎉 Data migration with ID mapping completed successfully!');
