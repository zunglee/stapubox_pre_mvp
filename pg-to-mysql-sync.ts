...
class PostgreSQLToMySQLSync {
  ...
  async testConnections(): Promise<boolean> {
    ...
    // Test PostgreSQL
    try {
      await this.pgDb.execute(sql`SELECT 1 as test`);
  ...
  async createMySQLTables(): Promise<void> {
  ...
  async syncUserActivities(): Promise<void> {
  ...
  async startPeriodicSync(intervalMinutes = 10): Promise<void> {
...
if (process.argv[1] && process.argv[1].includes('pg-to-mysql-sync')) {
